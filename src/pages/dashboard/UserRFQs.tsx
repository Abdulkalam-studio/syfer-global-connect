...
import { FileText, MessageSquare, Calendar, ChevronDown, ChevronUp, ArrowUpDown } from 'lucide-react';
  ...
  const userRFQs = rfqs
    ...
    .sort((a, b) => {
      if (sortField === 'createdAt') {
        ...
        const statusOrder = { 'Pending': 1, 'In Discussion': 2, 'Closed': 3 };
                    ...
                    <>
                      ...
                      >
                        ...
                        <td className="py-4 px-6">
                          <div>
                            <p className="text-foreground font-medium">{rfq.product?.name || 'Unknown'}</p>
                            <p className="text-xs text-muted-foreground">{rfq.product?.category}</p>
                          </div>
                        </td>
                        <td className="py-4 px-6 text-foreground">{rfq.quantity.toLocaleString()}</td>
                        <td className="py-4 px-6 text-foreground">{rfq.targetPrice || 'N/A'}</td>
                        <td className="py-4 px-6">
